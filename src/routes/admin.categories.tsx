        ...
        <Modal onClose={() => setEditing(null)} title={editing.id ? "Редактировать" : "Новая категория"}>
          <div className="space-y-3">
            ...
            <label className="block"><span className="text-xs text-neutral-600">Сортировка</span>
              <input placeholder="0" type="number" className="w-full mt-1 px-3 py-2 rounded-xl border" value={(editing.sort_order ?? 0) === 0 || (editing.sort_order ?? 0) === "0" ? "" : (editing.sort_order ?? 0)} onChange={(e) => setEditing({ ...editing, sort_order: Number(e.target.value) })} /></label>
