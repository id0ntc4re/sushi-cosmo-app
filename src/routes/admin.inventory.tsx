  ...
  return (
    <div className="bg-white rounded-3xl p-5">
      <div className="grid grid-cols-12 gap-2 mb-4 items-end">
        ...
        <input type="number" className={inp + " col-span-2"} placeholder="Остаток" value={(draft.stock) === 0 || (draft.stock) === "0" ? "" : (draft.stock)} onChange={(e) => setDraft({ ...draft, stock: Number(e.target.value) })} />
        <input type="number" className={inp + " col-span-2"} placeholder="Мин." value={(draft.min_stock) === 0 || (draft.min_stock) === "0" ? "" : (draft.min_stock)} onChange={(e) => setDraft({ ...draft, min_stock: Number(e.target.value) })} />
        <input type="number" className={inp + " col-span-2"} placeholder="Цена ₽/ед." value={(draft.cost_price) === 0 || (draft.cost_price) === "0" ? "" : (draft.cost_price)} onChange={(e) => setDraft({ ...draft, cost_price: Number(e.target.value) })} />
  ...
  return (
    <div className="grid lg:grid-cols-2 gap-5">
      ...
      <div className="bg-white rounded-3xl p-5">
        ...
        <input placeholder="0" type="number" step="0.01" value={(qty) === 0 || (qty) === "0" ? "" : (qty)} onChange={(e) => setQty(Number(e.target.value))} placeholder="Кол-во"
          className={inp + " mb-3"} />
      ...
      {!posted && (
        <>
          <div className="grid grid-cols-12 gap-2 mb-4 bg-neutral-50 p-3 rounded-xl">
            ...
            <input type="number" placeholder="Кол-во" value={(newItem.qty || "") === 0 || (newItem.qty || "") === "0" ? "" : (newItem.qty || "")} onChange={(e) => setNewItem({ ...newItem, qty: Number(e.target.value) })} className={inp + " col-span-2"} />
            <input type="number" step="0.01" placeholder="Цена за ед." value={(newItem.price || "") === 0 || (newItem.price || "") === "0" ? "" : (newItem.price || "")} onChange={(e) => setNewItem({ ...newItem, price: Number(e.target.value) })} className={inp + " col-span-2"} />
