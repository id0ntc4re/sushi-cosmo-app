        ...
        <Modal onClose={() => setEditing(null)} title={editing.id ? "Редактировать" : "Новая публикация"}>
          <div className="grid md:grid-cols-2 gap-3">
            ...
            <label className="block"><span className="text-xs text-neutral-600 block mb-1">Сортировка</span>
              <input placeholder="0" type="number" className={inp} value={(editing.sort_order ?? 0) === 0 || (editing.sort_order ?? 0) === "0" ? "" : (editing.sort_order ?? 0)} onChange={(e) => setEditing({ ...editing, sort_order: Number(e.target.value) })} /></label>
