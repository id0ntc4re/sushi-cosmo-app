...
import { formatDeliveryTime } from "@/lib/datetime";
          ...
          {!editing ? (
            ...
            <div className="bg-blue-50/50 rounded-2xl p-4 mb-5 space-y-3">
              ...
              <div className="grid grid-cols-2 gap-3 text-sm">
                ...
                <L lab="Персон"><input placeholder="0" type="number" value={(meta.persons) === 0 || (meta.persons) === "0" ? "" : (meta.persons)} onChange={(e) => setMeta({ ...meta, persons: Number(e.target.value) })} className={inp} /></L>
                <L lab="Сдача с"><input placeholder="0" type="number" value={(meta.change_from) === 0 || (meta.change_from) === "0" ? "" : (meta.change_from)} onChange={(e) => setMeta({ ...meta, change_from: e.target.value })} className={inp} /></L>
                <L lab="Доставка ₽"><input placeholder="0" type="number" value={(meta.delivery_cost) === 0 || (meta.delivery_cost) === "0" ? "" : (meta.delivery_cost)} onChange={(e) => setMeta({ ...meta, delivery_cost: Number(e.target.value) })} className={inp} /></L>
                <L lab="Скидка">
                  {(() => {
