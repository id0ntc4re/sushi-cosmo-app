          ...
          <div className="bg-white rounded-3xl p-6 max-w-lg w-full space-y-3 max-h-[90vh] overflow-y-auto" onClick={(e) => e.stopPropagation()}>
            ...
            {editing.discount_type !== "gift" && (
              <Field label="Размер скидки">
                <input placeholder="0" type="number" className={inp} value={(editing.discount_value) === 0 || (editing.discount_value) === "0" ? "" : (editing.discount_value)} onChange={(e) => setEditing({ ...editing, discount_value: e.target.value })} />
              </Field>
            )}
            ...
            <Field label="Мин. сумма заказа, ₽"><input placeholder="0" type="number" className={inp} value={(editing.min_order) === 0 || (editing.min_order) === "0" ? "" : (editing.min_order)} onChange={(e) => setEditing({ ...editing, min_order: e.target.value })} /></Field>
            ...
            <Field label="Макс. использований (пусто = без лимита)"><input placeholder="0" type="number" className={inp} value={(editing.max_uses) === 0 || (editing.max_uses) === "0" ? "" : (editing.max_uses)} onChange={(e) => setEditing({ ...editing, max_uses: e.target.value })} /></Field>
