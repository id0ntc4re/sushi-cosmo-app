    ...
    <div className="max-w-2xl">
      ...
      <div className="bg-white rounded-3xl p-6 space-y-4 mb-6">
        ...
        <Field label="Стоимость доставки, ₽">
          <input placeholder="0" type="number" className={inp} value={(s.delivery_cost) === 0 || (s.delivery_cost) === "0" ? "" : (s.delivery_cost)} onChange={(e) => setS({ ...s, delivery_cost: Number(e.target.value) })} />
        </Field>
        <Field label="Бесплатная доставка от, ₽">
          <input placeholder="0" type="number" className={inp} value={(s.free_delivery_from) === 0 || (s.free_delivery_from) === "0" ? "" : (s.free_delivery_from)} onChange={(e) => setS({ ...s, free_delivery_from: Number(e.target.value) })} />
        </Field>
        <Field label="Минимальная сумма заказа, ₽">
          <input placeholder="0" type="number" className={inp} value={(s.min_order) === 0 || (s.min_order) === "0" ? "" : (s.min_order)} onChange={(e) => setS({ ...s, min_order: Number(e.target.value) })} />
        </Field>
      ...
      <div className="bg-white rounded-3xl p-6 space-y-4 mb-6">
        ...
        <div className="grid grid-cols-2 gap-3">
          <Field label="Скидка, %">
            <input placeholder="0" type="number" className={inp} value={(flash.percent) === 0 || (flash.percent) === "0" ? "" : (flash.percent)}
