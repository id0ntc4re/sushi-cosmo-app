  ...
  useEffect(() => {
    (async () => {
      const [{ data: st }, { data: ad }, { data: br }, { data: zn }] = await Promise.all([
        ...
        (supabase.from("branches_public") as any).select("id,name,address").eq("is_active", true).order("sort_order"),
              ...
              <form onSubmit={submit} className="space-y-6">
                ...
                <Section title="Оплата">
                  ...
                  {form.payment_method === "cash" && (
                    <Field label="Сдача с (₽)">
                      <input min="0" type="number" className={inputCls} value={(form.change_from) || ""}
